...
void applyDamageToLocation(GameState& game, Unit* target, ArmorLocation location, int damage) {
	...
	if (remainingDamage > 0 && structLocation.currentStructure > 0) {
		...
		gamelogic::addLogMessage(game, "[DAMAGE] " + locationToString(structLoc) + " structure: " + std::to_string(structLocation.currentStructure + structureAbsorbed) + " → " + std::to_string(structLocation.currentStructure) + " (absorbed " + std::to_string(structureAbsorbed) + ")");
	}
	...
	if (structLocation.currentStructure <= 0 && !structLocation.isDestroyed) {
		...
		gamelogic::addLogMessage(game, "[LOCATION DESTROYED] " + locationToString(structLoc) + " destroyed!");

		// Death conditions
		if (structLoc == ArmorLocation::HEAD || structLoc == ArmorLocation::CENTER_TORSO) {
			gamelogic::addLogMessage(game, "[MECH DEATH] Critical location destroyed!");
			return;
		}

		// Both legs destroyed
		if (target->locations[ArmorLocation::LEFT_LEG].isDestroyed && target->locations[ArmorLocation::RIGHT_LEG].isDestroyed) {
			gamelogic::addLogMessage(game, "[MECH DEATH] Both legs destroyed!");
			return;
		}

		// Single leg loss
		if (structLoc == ArmorLocation::LEFT_LEG || structLoc == ArmorLocation::RIGHT_LEG) {
	...
	// Log final status
	gamelogic::addLogMessage(game, "[LOCATION STATUS] " + locationToString(location) + ": " + std::to_string(loc.currentArmor) + "/" + std::to_string(loc.maxArmor) + " armor, " + std::to_string(structLocation.currentStructure) + "/" + std::to_string(structLocation.maxStructure) + " structure");
}
