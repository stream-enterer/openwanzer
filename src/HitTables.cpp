...
ArmorLocation rollHitLocation(combatarcs::AttackArc arc) {
	...
	switch (arc) {
		case combatarcs::AttackArc::FRONT:
			...
			if (roll < 55)
				return ArmorLocation::LEFT_ARM;
			if (roll < 65)
				return ArmorLocation::RIGHT_ARM;
			if (roll < 73)
		...
		case combatarcs::AttackArc::REAR:
			if (roll < 16)
				return ArmorLocation::CENTER_TORSO_REAR;
			if (roll < 30)
				return ArmorLocation::LEFT_TORSO_REAR;
			if (roll < 44)
				return ArmorLocation::RIGHT_TORSO_REAR;
			if (roll < 48)
				return ArmorLocation::LEFT_ARM;
			if (roll < 52)
				return ArmorLocation::RIGHT_ARM;
